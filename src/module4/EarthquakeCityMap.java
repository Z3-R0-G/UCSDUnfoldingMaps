	...
	private void printQuakes() {
		...
		for (Marker cm: countryMarkers) {
			...
			for (PointFeature eq: earthquakes) {
				if (isInCountry(eq, cm)) {
					if ((String)eq.getProperty("country") == name) {
						country_counter = country_counter + 1;
					}
				}
			}
			if (country_counter != 0) {
				System.out.println(name + ": " + String.valueOf(country_counter));
				total = total + country_counter;
			}
		}
		System.out.println("OCEANQUAKES: " + String.valueOf(earthquakes.size() - total));
